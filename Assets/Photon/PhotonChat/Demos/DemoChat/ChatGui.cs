...
namespace Photon.Chat.Demo
{
    ...
    public class ChatGui : MonoBehaviour, IChatClientListener
    {
        ...
        private void SendChatMessage(string inputLine)
        {
            ...
            if (false)//inputLine[0].Equals('\\'))//変更(false)//
            {
                ...
                else if ((tokens[0].Equals("\\subscribe") || tokens[0].Equals("\\s")) && !string.IsNullOrEmpty(tokens[1]))
                {
                    ...
                    for (int i = 0; i < tokens[1].Split(new char[] { ' ', ',' }).Length; i++)
                    {
                        this.chatClient.Subscribe(tokens[1].Split(new char[] { ' ', ',' })[i], 0, -1, channelCreationOptions);
                    }
                }
        ...
        public void ChangeChannel(string previousChannelName, string channelName)
        {
            chatClient.Unsubscribe(new string[] { previousChannelName });
            var channelCreationOptions = new ChannelCreationOptions();
            channelCreationOptions.PublishSubscribers = true;
            channelCreationOptions.MaxSubscribers = 100;
            chatClient.Subscribe(channelName, 0, -1, channelCreationOptions);
        }
        ...
        public void OnConnected()
        {
            ...
            if (this.ChannelsToJoinOnConnect != null && this.ChannelsToJoinOnConnect.Length > 0)
            {
                ...
                {
                    this.chatClient.Subscribe(this.ChannelsToJoinOnConnect[i], 0, this.HistoryLengthToFetch, channelCreationOptions);
                }
            }
            this.ConnectingLabel.SetActive(false);
        ...
        private void InstantiateChannelButton(string channelName)
        {
            ...
            //↑から↓へ変更した
            cbtns.Add((Toggle)Instantiate(this.ChannelToggleToInstantiate));
            cbtns[Math.Max(0, cbtns.Count - 1)].gameObject.SetActive(true);
            cbtns[Math.Max(0, cbtns.Count - 1)].GetComponentInChildren<ChannelSelector>().SetChannel(this, channelName);
            cbtns[Math.Max(0, cbtns.Count - 1)].transform.SetParent(this.ChannelToggleToInstantiate.transform.parent, false);

            this.channelToggles.Add(channelName, cbtns[Math.Max(0, cbtns.Count - 1)]);
        }
        List<Toggle> cbtns = new List<Toggle>();
